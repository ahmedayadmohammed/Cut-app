...
//    GET ALL EMPLOYEE FUNCTION BY POSTION ID
    func GETALLEMPLOYEEBYID(){
        ...
        httpRequest(vc:self, url: get.root.GET_EMPLOYEE_BY_POSITIONS_ID!, httpMethod: .get, parameters:param, headers: self.token) { [weak self](rest:Swift.Result<Getemployee,Error>?) in
            if let output = rest{
                switch output{
                case .success(let ok):
